...
- The provider now honors user gestures before starting a download, pipes `createSessionWithProgress` callbacks into `RoundProgressBar` as 0–100 percentages, and retries once after clearing the Transformers.js Cache API + IndexedDB buckets so a corrupted cache doesn’t brick the UX.
