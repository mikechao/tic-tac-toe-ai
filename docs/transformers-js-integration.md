...
- The provider now honors user gestures before starting a download, pipes `createSessionWithProgress` callbacks into `RoundProgressBar` as 0–100 percentages, retries once after clearing the Transformers.js Cache API + IndexedDB buckets, and logs download telemetry (duration, estimated size, attempt count, failures) to Sentry so ops can trace SmolLM2 issues just like the Gemini path.
- Game loop guardrails: only one SmolLM2 inference runs at a time. `TransformersJSProvider` tracks an `isInferenceActive` flag; the GameLoop aborts new turns if SmolLM2 is already working, emits a user toast (“SmolLM2 is finishing a move”), and MatchControls disables the start button until the current inference finishes. Browsers without WebGPU short-circuit with a friendly error instead of falling back to server inference.
...
- `apps/frontend/src/hooks/useTransformersModel.ts` exposes a hook that instantiates `transformersJS("HuggingFaceTB/SmolLM2-360M-Instruct", { worker, device: 'webgpu', initProgressCallback })`, relays `availability()`/`createSessionWithProgress()` calls, and forwards progress events (converted from 0-1 to 0-100) into the shared `TransformersJSProvider` state.
- `apps/frontend/src/lib/game/ai-turn.ts` now has a shared `requestMoveWithResolver` helper so both Gemini and SmolLM2 go through the same `generateObject` pipeline. When a SmolLM2-backed match runs, `GameLoopProvider` calls `requestTransformersMove`, which pulls the cached Transformers.js model, enforces `doesBrowserSupportTransformersJS()`, and aborts with a “WebGPU required” message instead of falling back to server inference.
