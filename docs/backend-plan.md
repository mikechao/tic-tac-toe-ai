...
- [x] Run `pnpm --filter backend db:generate` to sync SQL migrations; commit generated files for traceability
- [x] Seed reference data (AI models, starter accounts) via `db:seed` hitting Xata dev branch
...
| `moves` | `id`, `round_id`, `turn_index`, `cell`, `symbol`, `elapsed_ms` | Append-only child table linked to each round via `round_id` |
...
| `match_id` | uuid | Shared identifier across rounds; minted on the first submission and reused for the same session |
